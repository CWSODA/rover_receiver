...
void LidarParser::parse_byte(uint8_t byte, LidarDrawer& lidar_drawer) {
    ...
    switch (state) {
        ...
        case (State::DATA): {
            if (parse_data(byte, lidar_drawer)) state = State::CHECKSUM;
            break;
        }
        case (State::HEALTH): {
            ++data_byte_count;
...
bool LidarParser::parse_data(uint8_t byte, LidarDrawer& lidar_drawer) {
    ++data_byte_count;
    switch (data_state) {
...
// debug stuff
        ...
        case (DataState::DIST): {
            ...
            temp_point.distance = (float)dist_buf.val() * 0.25E-3f;
            ...
            count++;
#endif
            // restructure dist and sig strength
            // str, dist, dist
            // dist, dist, str
            // uint8_t first_byte = temp_point.sig_strength;
            // temp_point.sig_strength = dist_buf.val() & 0xF;
            // uint16_t revised_dist =
            //     (first_byte << 8) + ((dist_buf.val() & 0xF0) >> 8);
            // temp_point.distance = revised_dist * 0.25E-3f;

            // add points to lidar drawer
            lidar_drawer.add_point(temp_point.sig_strength, temp_point.distance,
                                   temp_point.angle);
...
void LidarParser::reset_state() {
    ...
    // if (checksum_total != checksum_buf.val()) {
    //     DBG("INVALID CS!\n");
    //     data_vec.clear();
    // }
